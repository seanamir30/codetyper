...
function App() {
  ...
  return (
    <main className="w-full min-h-screen relative flex flex-col items-center justify-between bg-neutral-900">
      ...
      {/* Main Content */}
      <section className="flex flex-col items-center gap-6 my-12 w-full xl:max-w-5xl px-4">
      ...
      <section className="flex flex-col xl:flex-row items-center xl:items-start px-4 sm:px-8 xl:px-0 gap-10 relative w-full xl:max-w-5xl">
        ...
        <aside className="flex flex-col text-white w-64 flex-shrink-0 gap-6" role="complementary" aria-label="Detailed typing statistics">
          <h2 className="text-4xl font-medium text-center" aria-label={`Current speed: ${currentCPM} characters per minute`}>
        ...
        <div className="flex flex-col gap-6">
          ...
          <fieldset className="flex flex-col items-center xl:items-start gap-2">
            <legend className="text-white text-2xl font-semibold">Programming Languages</legend>
              ...
              {languageKeys.map((languageKey)=>{
                return(
                  <button 
                    ...
                    className={`transition text-neutral-400 bg-neutral-800 text-xl hover:text-white px-6 py-2 rounded-md ${languageKey === options.language && '!bg-neutral-700 !text-white'}`}
                    role="radio"
          ...
          {/* Duration Selection */}
          <fieldset className="flex flex-col items-center xl:items-start gap-2">
            <legend className="text-white text-2xl font-semibold">Test Duration</legend>
            <div className="flex flex-wrap justify-center xl:justify-start gap-2" role="radiogroup" aria-label="Select test duration">
              {durations.map((duration)=>(
                <button 
                  key={duration}
                  className={`transition text-neutral-400 bg-neutral-800 text-xl hover:text-white px-6 py-2 rounded-md ${duration === options.seconds && '!bg-neutral-700 !text-white'}`} 
